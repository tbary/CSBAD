...
# def plot_two_embedding(Paths_sets, title="Embeddings: pairwise overlays vs Set1"):
#     """
#     Paths_sets: sequence of iterables[str | Path]
#         Each item is a *file path* to an embedding (e.g., .../airplane/0001_embedding.pt).
#         Class = parent folder name. Assumes sets i>0 are subsets of set 0 (by files/classes).

#     Behavior:
#       - If len(Paths_sets) == 2: single plot with Set1 (●) and Set2 (×).
#       - If len(Paths_sets)  > 2: create subplots; each panel overlays Set1 with Set{i} (i>=2).

#     Returns:
#       dict with:
#         - 'Z_list': list[np.ndarray], projected coords per set (n_i, 2)
#         - 'y_list': list[np.ndarray], class indices per set (n_i,)
#         - 'classes': list[str], class names
#         - 'pca_mean': (1, D)
#         - 'pca_vt': (D, D)
#         - 'counts_list': list[dict], per-set counts per class
...
#     # --- Coerce & filter existing files per set ---
#     files_sets = []
#     for s in Paths_sets:
...
#     # --- Build union of class names for stable color map ---
#     all_classes = set()
...
#     # --- Helper: reuse your load_one_file ---
...
#                 continue
#             arr = load_one_file(fp)  # your function
...
#     Z_list = []
#     for X in X_list:
#         Z = (X.astype(np.float64) - mean) @ W2 if X.size else np.empty((0, 2))
#         Z_list.append(Z)

#     # --- Plotting setup ---
#     cmap = plt.get_cmap("tab10" if len(class_names) <= 10 else "tab20")

#     # Marker/style choices
#     set1_marker = 'o'
#     set1_size   = 10
#     set1_alpha  = 0.35
#     set1_lw     = 0.0
...
#         set_handles = [
#             Line2D([0],[0], marker=set1_marker, linestyle='None', color='k',
#                    label='Set 1 (●)', markersize=6),
#             Line2D([0],[0], marker=other_marker, linestyle='None', color='k',
#                    label='Set 2 (×)', markersize=7),
#         ]
#         ax.legend(handles=set_handles, title="Dataset", loc='lower right',
#                   fontsize=9, frameon=False)
...
#         return {
#             "Z_list": Z_list, "y_list": y_list, "classes": class_names,
#             "pca_mean": mean, "pca_vt": vt, "counts_list": counts_list,
#             "figure": fig, "axes": ax
#         }

#     # --- Case B: more than 2 sets -> subplots comparing set1 with each other set ---
#     n_panels = len(Paths_sets) - 1
#     cols = min(3, n_panels)              # up to 3 columns
...
#             # Always plot Set1 as faint colored circles
#             for ci, _cname in enumerate(class_names):
#                 m1 = (y_list[0] == ci)
#                 if m1.any():
#                     ax.scatter(Z_list[0][m1, 0], Z_list[0][m1, 1],
...
#             # Overlay the current set (i = panel_idx+1) as colored crosses
#             i = panel_idx + 1
#             for ci, _cname in enumerate(class_names):
#                 mi = (y_list[i] == ci)
#                 if mi.any():
...
#     # Shared legends (outside panels)
#     class_handles = [Line2D([0],[0], marker='o', linestyle='None',
#                             color=cmap(i), label=cname, markersize=6)
#                      for i, cname in enumerate(class_names)]
#     legend1 = fig.legend(handles=class_handles, title="Class", loc="upper right",
#                          bbox_to_anchor=(0.98, 0.98), fontsize=9, frameon=False)

#     set_handles = [
#         Line2D([0],[0], marker=set1_marker, linestyle='None', color='k',
#                label='Set 1 (●)', markersize=7),
#         Line2D([0],[0], marker=other_marker, linestyle='None', color='k',
#                label='Compared set (×)', markersize=8),
#     ]
#     legend2 = fig.legend(handles=set_handles, title="Dataset", loc="lower right",
#                          bbox_to_anchor=(0.98, 0.02), fontsize=9, frameon=False)

#     fig.suptitle(title, y=0.995, fontsize=12)
#     plt.tight_layout(rect=[0.02, 0.04, 0.96, 0.96])  # leave room for legends/suptitle
#     plt.show()
