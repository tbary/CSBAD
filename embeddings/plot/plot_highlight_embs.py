...
def parse_highlight_keys(raw: str | None) -> list[str]:
    ...
    if s.startswith("["):
        ...
        return [str(x).strip().strip("'").strip('"').strip() for x in v]
...
def main():
    ...
    ap.add_argument("--markers-per-key", action="store_true",
                    help="If set and using --highlight-keys: use a different marker for each key")
    ap.add_argument("--legend", action="store_true", help="Show legend (useful with --markers-per-key)")
    ...
    # background (everything grey by default)
    plt.figure(figsize=(9, 7))
    plt.scatter(Y[:, 0], Y[:, 1], s=6, alpha=0.12, c="lightgrey")

    marker_cycle = ["o", "s", "^", "v", "D", "P", "X", "*", "<", ">", "h", "8", "p"]

    if args.highlight_keys is not None:
        keys = parse_highlight_keys(args.highlight_keys)
        if not keys:
            raise ValueError("Empty --highlight-keys")
